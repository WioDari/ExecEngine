...
def _install_signal_handlers(stop_event: asyncio.Event):
    ...
    def request_stop() -> None:
        logger.info("Callback worker shutdown requested")
        stop_event.set()
...
async def async_main():
    ...
    validate_role_configuration(load_ini(), "callback")
    settings = CallbackSettings.from_ini()
    ...
    await CallbackDispatcher(settings, SessionLocal).run(stop_event)


def main():
    from app.core.logger import setup_logging

    setup_logging("callback-worker")
