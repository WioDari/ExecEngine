...
async def create_batch_submission(batch_submission: BatchSubmissionCreate, db: Session = Depends(get_db), current_user: UserModel = Depends(get_current_user)):
...
async def get_batch_submissions(batch_token: str, db: Session = Depends(get_db), current_user: UserModel = Depends(get_current_user)):
    batch = db.query(BatchModel).filter(BatchModel.batch_token == batch_token).first()
    if not batch:
        raise HTTPException(status_code=404, detail="Batch not found.")

    submissions = db.query(SubmissionModel).filter(SubmissionModel.batch_id == batch.id, SubmissionModel.user_id == current_user.id).all()
    response = []
