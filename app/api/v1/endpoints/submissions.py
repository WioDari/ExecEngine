...
async def create_submission(submission: SubmissionCreate, db: Session = Depends(get_db), current_user: UserModel = Depends(get_current_user)):
...
async def get_submission(token: str, db: Session = Depends(get_db), current_user: UserModel = Depends(get_current_user)):
    submission = db.query(SubmissionModel).filter(SubmissionModel.token == token, SubmissionModel.user_id == current_user.id).first()
    if not submission:
